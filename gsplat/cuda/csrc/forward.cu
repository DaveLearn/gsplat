...
__global__ void rasterize_surfel_indices(
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            // if the opacity of the guassian is less than the threshold (0.5we don't consider it solid.
            if (opac < opacity_threshold) {
                continue;
            }

            // if we are solid and impact the pixel, save the surfel id
            out_surfel_ids[pix_id] = id_batch[t];
            done = true;
        }
    }
}
